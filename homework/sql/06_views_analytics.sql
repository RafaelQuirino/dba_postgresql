...
FROM (
  SELECT 'movies' AS tipo, ps.nome, COUNT(*) AS total,
    ...
    JOIN analytics.equipe e ON m.producao_id = e.producao_id
    JOIN analytics.pessoa ps ON e.pessoa_id = ps.pessoa_id
    GROUP BY ps.nome
  UNION ALL
  SELECT 'tv_shows', ps.nome, COUNT(*),
         ROW_NUMBER() OVER (PARTITION BY 'tv_shows' ORDER BY COUNT(*) DESC)
    FROM analytics.tv_shows t
    JOIN analytics.equipe e ON t.producao_id = e.producao_id
    JOIN analytics.pessoa ps ON e.pessoa_id = ps.pessoa_id
    GROUP BY ps.nome
  UNION ALL
  SELECT 'video_games', ps.nome, COUNT(*),
    ...
    JOIN analytics.equipe e ON v.producao_id = e.producao_id
    JOIN analytics.pessoa ps ON e.pessoa_id = ps.pessoa_id
    GROUP BY ps.nome
  UNION ALL
  SELECT 'documentaries', ps.nome, COUNT(*),
    ...
    JOIN analytics.equipe e ON d.producao_id = e.producao_id
    JOIN analytics.pessoa ps ON e.pessoa_id = ps.pessoa_id
    GROUP BY ps.nome
  UNION ALL
  SELECT 'short_films', ps.nome, COUNT(*),
    ...
    JOIN analytics.equipe e ON s.producao_id = e.producao_id
    JOIN analytics.pessoa ps ON e.pessoa_id = ps.pessoa_id
    GROUP BY ps.nome
  UNION ALL
  SELECT 'music_videos', ps.nome, COUNT(*),
    ...
    JOIN analytics.equipe e ON mv.producao_id = e.producao_id
    JOIN analytics.pessoa ps ON e.pessoa_id = ps.pessoa_id
    GROUP BY ps.nome
  UNION ALL
  SELECT 'theater_productions', ps.nome, COUNT(*),
    ...
    JOIN analytics.equipe e ON th.producao_id = e.producao_id
    JOIN analytics.pessoa ps ON e.pessoa_id = ps.pessoa_id
    GROUP BY ps.nome
  UNION ALL
  SELECT 'web_series', ps.nome, COUNT(*),
    ...
    JOIN analytics.equipe e ON ws.producao_id = e.producao_id
    JOIN analytics.pessoa ps ON e.pessoa_id = ps.pessoa_id
    GROUP BY ps.nome
  UNION ALL
  SELECT 'animations', ps.nome, COUNT(*),
    ...
    JOIN analytics.equipe e ON a.producao_id = e.producao_id
    JOIN analytics.pessoa ps ON e.pessoa_id = ps.pessoa_id
    GROUP BY ps.nome
) sub
WHERE rn <= 10;
...
JOIN analytics.equipe e ON p.producao_id = e.producao_id
